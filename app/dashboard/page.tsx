...
function MethodologyDocsEditor({ supabase, profile }: any) {
  ...
  return (<>
    ...
    {(adding || editId) && (<div style={{ marginBottom: 14, padding: 14, background: '#0c1017', borderRadius: 10, border: '1px solid #2a3140' }}>
      {adding && (<div style={{ marginBottom: 12, padding: 12, background: '#111621', borderRadius: 8, border: '1px dashed #2a3140' }}><div style={{ fontSize: 11, color: '#8b95a5', marginBottom: 6 }}>📎 Importer un fichier <span style={{ color: '#5a6578' }}>(optionnel — PDF, DOCX, TXT, max 10 MB)</span></div><input type="file" accept=".pdf,.docx,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain" onChange={handleFileUpload} disabled={uploading} style={{ fontSize: 12, color: '#c9c9d0', width: '100%' }} />{uploading && <div style={{ fontSize: 11, color: '#3b82f6', marginTop: 8 }}>⏳ Extraction en cours…</div>}{uploadError && <div style={{ fontSize: 11, color: '#ef4444', marginTop: 8 }}>❌ {uploadError}</div>}</div>)}<input value={editName} onChange={e => setEditName(e.target.value)} placeholder="Nom du document" style={{ ...iS, marginBottom: 8 } as any} />
      ...
      <div style={{ position: "fixed", left: 0, top: 0, width: 220, height: "100vh", background: "#111621", borderRight: "1px solid #1e2530", display: "flex", flexDirection: "column", zIndex: 100, transform: isMobile && !drawerOpen ? "translateX(-100%)" : "translateX(0)", transition: "transform 0.25s ease-out" }}>
        ...
        {org?.status === 'trialing' && (profile?.role === 'admin' || profile?.role === 'super_admin') && (
          <div style={{ padding: "14px 16px", borderTop: "1px solid #1e2530", background: "linear-gradient(135deg, rgba(99,195,151,0.12), rgba(99,195,151,0.04))" }}>
            ...
            <div style={{ fontSize: 12, color: "#e8eaed", lineHeight: 1.5, marginBottom: 6 }}>{(() => { const d = org?.trial_ends_at ? Math.max(0, Math.ceil((new Date(org.trial_ends_at).getTime() - Date.now()) / 86400000)) : 7; return (<>Encore <strong style={{ color: "#63c397" }}>{d}</strong> jour{d > 1 ? 's' : ''} avant début de la facturation.</>); })()}</div>
...
function ChatSession({ profile, personas, formations, scoring, config, sd, supabase, onEnd, onCancel, saleDocuments }: any) {
  ...
  const finish = useCallback(async (m: any[], r: string) => { setAnalyzing(true);
  ...
  if (analyzing) return (<div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "#0a0a0b", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "32px", zIndex: 9999 }}><div style={{ fontSize: 28, fontWeight: 500, color: "#e8eaed", textAlign: "center", maxWidth: 600, padding: "0 20px", lineHeight: 1.4 }}>Analyse de votre session en préparation, veuillez patienter...</div><svg width={48} height={48} viewBox="0 0 50 50"><circle cx={25} cy={25} r={20} fill="none" stroke="#2a3140" strokeWidth={4} /><circle cx={25} cy={25} r={20} fill="none" stroke="#63c397" strokeWidth={4} strokeDasharray="60 50" strokeLinecap="round"><animateTransform attributeName="transform" type="rotate" from="0 25 25" to="360 25 25" dur="1s" repeatCount="indefinite" /></circle></svg></div>)
  ...
  return (<div style={{ padding: isMobile ? "20px 16px" : "32px 40px", maxWidth: 900 }}><div style={{ fontSize: isMobile ? 20 : 22, fontWeight: 800, marginBottom: isMobile ? 18 : 24 }}>Historique{isAdmin ? " (toutes)" : ""}</div>
    {sessions.filter((s: any) => s.result !== 'in_progress').length === 0 ? <div style={{ textAlign: "center", padding: 40, color: "#8b95a5" }}>Aucune session</div> : sessions.filter((s: any) => s.result !== 'in_progress').map((s: any) => { const p = personas.find((x: any) => x.id === s.persona_id); const f = formations.find((x: any) => x.id === s.formation_id); const u = profiles.find((x: any) => x.id === s.vendor_id); return <div key={s.id} style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "center", justifyContent: "space-between", gap: isMobile ? 12 : 0, padding: isMobile ? "12px 14px" : "14px 18px", background: "#111621", borderRadius: 12, border: "1px solid #1e2530", marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: isMobile ? "wrap" : "nowrap", justifyContent: isMobile ? "space-between" : "flex-end" }}>
        <button onClick={(e)=>{e.stopPropagation(); onView(s)}} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 14px",background:"rgba(99,195,151,0.1)",border:"1px solid rgba(99,195,151,0.4)",borderRadius:8,color:"#63c397",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>Voir le rapport</button>
